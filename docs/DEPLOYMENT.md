...
Sejak Phase 08, seluruh titik notifikasi memanggil `pkg/kirimdev` dengan **nama template literal** berikut (dikonfirmasi ada di kode, bukan hanya di techspec — lihat `grep` di setiap file). **Tak satu pun dari 13 nama ini pernah didaftarkan/disetujui** di dashboard Kirimdev/Meta Business Manager — techspec §14.3 eksplisit mencatat "Nama template **belum final** — harus didaftarkan & disetujui ... sebelum implementasi (di luar kendali TechSpec ini)". Ini adalah blocker go-live paling konkret: tanpa registrasi, `kirimdevClient.Deliver()` akan gagal (template not found) untuk semua job WhatsApp, retry 5x lalu masuk dead-letter — pengguna tidak pernah menerima notifikasi apa pun.

| # | Nama Template | Event | Call site | Params (urutan) |
|---|---|---|---|---|
| 1 | `invoice_donasi_kantong_amal` | Donasi dibuat (instruksi QRIS) | `donation_service_impl.go:160` | nominal, nama campaign, link QR |
| 2 | `donasi_berhasil_kantong_amal` | Donasi lunas (ke donor) | `donation_service_impl.go:343` | nama donor, nominal, nama campaign |
| 3 | `notifikasi_pic_donasi_kantong_amal` | Donasi lunas (ke owner campaign) | `donation_service_impl.go:359` | nominal, nama donor (respect anonymous), nama campaign |
| 4 | `campaign_disetujui` | Campaign disetujui | `campaign_service_impl.go:394` | nama campaign |
| 5 | `campaign_revisi` | Campaign perlu revisi | `campaign_service_impl.go:396` | nama campaign, catatan reviewer |
| 6 | `campaign_ditolak` | Campaign ditolak | `campaign_service_impl.go:398` | nama campaign, catatan reviewer |
| 7 | `withdrawal_diajukan` | Withdrawal diajukan | `withdrawal_service_impl.go:252` | nominal, nama campaign |
| 8 | `kode_otp_kantong_amal` | Verifikasi keamanan (OTP) | `withdrawal_service_impl.go:340` | kode OTP, masa berlaku |
| 9 | `withdrawal_disetujui` | Withdrawal disetujui | `withdrawal_service_impl.go:446` | nominal |
| 10 | `withdrawal_diproses` | Withdrawal diproses | `withdrawal_service_impl.go:546` | nominal, estimasi waktu |
| 11 | `withdrawal_berhasil` | Withdrawal berhasil cair | `withdrawal_service_impl.go:616` | nominal, rekening tujuan |
| 12 | `withdrawal_gagal` | Withdrawal gagal/ditolak | `withdrawal_service_impl.go:478,515,623` | nominal, alasan |
| 13 | `rekening_diubah` | Beneficiary diubah (alert keamanan) | `campaign_service_impl.go:304` | rekening baru (masked), waktu |

**Sebelum go-live**:
- [ ] Semua 13 template di atas dibuat & **disetujui (approved)** di Meta Business Manager untuk nomor WhatsApp Business production.
...
- [ ] `KIRIMDEV_TEMPLATE_LANGUAGE=id` cocok dengan bahasa template yang didaftarkan.
