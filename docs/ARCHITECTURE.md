...
>
> **`qrcode`** menggantikan peran "tautan pendek" dengan gambar QR yang meng-encode `destinationURL` **langsung** (tanpa kunci/redirect/pelacakan pindaian — kalau ada itu semua, sama saja dengan `shortlink`). Foldernya memakai bentuk dua-entity-satu-modul yang sama dengan `shortlink`: `qrcode_*` (CRUD terkelola staff + generate gambar PNG server-side) ditambah `qrcoderequest_*` (alur permintaan publik + persetujuan admin) di subfolder terpisah dalam folder modul yang sama — `qrcoderequest_service` mereplikasi persis pola dua-jalur-approval race-safe & notifikasi lewat job queue milik `shortlinkrequest_service` (lihat §12 di bawah). Detail endpoint: [API.md §8i–§8j](./API.md#8i-qr-code-qrcodes).
...
| `0008_setting.up.sql` | Tabel `ms_setting` (App Settings, konfigurasi runtime key-value generik) + seed PIC shortlink + permission `setting.*` → **Super Admin only** |
...
## 12. Permintaan Shortlink: Dua Jalur Approval Race-Safe, Notifikasi via Job Queue

> **Revisi** — bagian ini awalnya mendeskripsikan "satu jalur approval saja" dengan webhook Kirimdev log-only. Setelah audit terhadap aplikasi referensi fitur ini, desainnya diperluas: sekarang ada **dua jalur approval** (CMS & balasan WhatsApp), dibuat race-safe lewat mekanisme atomik bersama, dan notifikasi pindah dari goroutine fire-and-forget ke job queue (§13) dengan retry. Baca §13 sebelum bagian ini untuk konteks job queue.

`modules/shortlink/shortlinkrequest_*` menambahkan alur publik-mengajukan/admin-menyetujui **di dalam** folder modul `shortlink` yang sudah ada (satu domain fitur, subfolder terpisah per entity — bukan modul baru), karena bergantung erat pada logic generate-key `shortlink_service` yang sudah ada.

**Dua jalur approval, satu mekanisme atomik.** Selain admin meng-klik Approve/Reject di CMS, PIC juga bisa menyelesaikan sebuah permintaan lewat balasan WhatsApp ("YES"/"NO" atau tombol quick-reply) — lihat `shortlinkrequest_service.HandleWhatsAppReply`, dipanggil dari `POST /public/webhooks/kirimdev` (bukan lagi log-only). **Kedua jalur ini saling kenal**: baik `Approve`/`Reject` (CMS) maupun `HandleWhatsAppReply` (WhatsApp) melewati dua method privat yang sama, `approveRequest`/`rejectRequest`, yang pada gilirannya memanggil `shortlinkrequest_repository.ApproveTx`/`UpdateStatus` — dan kedua method repository itu sekarang **kondisional**: `UPDATE ms_shortlink_request SET ... WHERE shortLinkRequestID=? AND status='pending'`, dicek `RowsAffected`. Kalau `RowsAffected == 0`, repository mengembalikan sentinel `ErrAlreadyProcessed` — berarti jalur lain (atau pemanggil lain di jalur yang sama) sudah lebih dulu menyelesaikan request ini. Klausa `WHERE` pada `UPDATE` itu sendiri sudah atomik di level MySQL (row lock implisit selama statement berjalan), jadi tidak perlu `SELECT ... FOR UPDATE` eksplisit — siapa pun yang menang race itu otentik, yang kalah dapat error yang jelas, dan `ApproveTx` men-*insert* `ms_shortlink` di transaksi yang sama sehingga baris shortlink ikut ter-rollback kalau update kondisionalnya kalah race (tidak ada baris shortlink nyasar).

**`ErrKeyCollision` menutup celah duplicate-key.** Kalau dua permintaan (atau permintaan yang sama diproses dua jalur nyaris bersamaan) berebut `shortKey` yang sama, `ApproveTx` mendeteksi error MySQL 1062 (`Duplicate entry`, dicek via `errors.As` ke `*mysql.MySQLError`) pada insert `ms_shortlink` dan mengembalikan sentinel `ErrKeyCollision` alih-alih error generik — `Approve` (CMS) menerjemahkannya ke `409 Conflict`, sedangkan `HandleWhatsAppReply` (WhatsApp) membiarkan request tetap `pending` dan hanya mencatat log untuk ditindaklanjuti manual lewat CMS (WhatsApp Business API mengharuskan pesan business-initiated pakai template pra-approved, jadi tidak ada balasan WhatsApp otomatis untuk kasus ini).

**Resolusi balasan WhatsApp pakai reply-threading, bukan cache pointer.** `resolveTargetRequest` mencocokkan `context.id` pada inbound webhook (field WhatsApp yang menunjuk `wamid` pesan yang di-reply) ke tabel `tr_whatsapp_message_log` (§13) — tabel ini mencatat `waMessageID → correlationID` setiap kali job pengiriman WhatsApp ke PIC/requester selesai. Fallback (balasan tanpa reply/quote): cocokkan ke request `pending` milik nomor WhatsApp yang sama lewat `FindRecentByPhone`. **Kalau ada lebih dari satu kandidat pending, sistem TIDAK menebak** — mengembalikan `OutcomeAmbiguousOrNotFound` dan tidak melakukan apa pun, supaya tidak pernah salah memproses request yang keliru.

**Notifikasi lewat job queue (§13), bukan goroutine langsung.** `Submit`/`approveRequest`/`rejectRequest` memanggil `jobqueue_service.Enqueue(...)` (lewat interface sempit `JobEnqueuer`) alih-alih `go func(){ kirimdevClient.SendTemplate(...) }()` — kegagalan kirim sekarang dapat retry otomatis dengan backoff, bukan sekali-coba-lalu-log. `shortlinkrequest_service` **tidak lagi** memegang `*kirimdev.Client`/`mailer.Mailer` langsung.

**`SettingReader` & `JobEnqueuer`/`WhatsAppMessageResolver` — pola interface sempit yang sama dengan `CommentCleaner`.** `shortlinkrequest_service` membaca nomor/nama PIC dari modul `setting`, dan mengantre/resolve pesan WhatsApp dari modul `jobqueue`, lewat interface lokal kecil — bukan meng-import `setting_service`/`jobqueue_service` dengan tipe konkret. Idiom Go "accept interfaces, return structs" yang sama dipakai `article_service`/`news_service`/`event_service` terhadap `comment_service` (lihat §2). `setting_service.Service` dan `jobqueue_service.Service` otomatis memenuhi interface-interface ini tanpa perlu deklarasi eksplisit — satu nilai `jobqueueSvc` di root `router.go` dipakai untuk memuaskan **dua** interface (`JobEnqueuer` dan `WhatsAppMessageResolver`) sekaligus.

**`GET /public/shortlink-requests/pic` — subset publik App Settings, bukan endpoint `/settings` yang di-buka.** Halaman publik pengajuan (`fsldk-web`) menampilkan kartu "Konfirmasi via WhatsApp" berisi nama & nomor PIC supaya requester bisa langsung menyapa PIC via `wa.me` sambil menunggu ditinjau — ini di luar cakupan asli spesifikasi awal fitur ini (yang hanya menyebut field form + pesan sukses), ditambahkan belakangan atas permintaan eksplisit. Daripada membuka `GET /settings` (Super Admin only, §10a) ke publik, `shortlinkrequest_service.PublicPIC` membaca hanya dua key spesifik (`shortlink_pic_name`/`shortlink_pic_whatsapp`) lewat `SettingReader` yang sudah ada dan mengembalikannya lewat DTO sempit (`PICResponse`) — App Settings sendiri tetap 100% CMS-only, tidak ada baris kode yang membuka `/settings` ke luar CMS.

**Webhook Kirimdev terikat timestamp, bukan sekadar HMAC atas payload.** `POST /public/webhooks/kirimdev` memverifikasi signature HMAC-SHA256 atas `"{timestamp}.{rawBody}"` (bukan payload mentah saja), dicek terhadap **setiap** secret di `KIRIMDEV_WEBHOOK_SECRETS` yang comma-separated (mendukung rotasi tanpa downtime), DAN ditolak kalau timestamp-nya di luar `KIRIMDEV_REPLY_WINDOW_MINUTES` (default 5 menit) — mencegah replay attack pengiriman ulang payload lama. Handler selalu merespons `200 OK` kecuali signature gagal, supaya Kirimdev tidak retry-storm untuk outcome bisnis biasa (mis. "ignored", "ambiguous").

**Pola yang sama persis dipakai `modules/qrcode/qrcoderequest_*`.** Dua jalur approval (CMS `qrcode.approve` ATAU balasan WhatsApp PIC), `UPDATE ... WHERE status='pending'` atomik, `ErrAlreadyProcessed`, notifikasi lewat job queue (§13) — satu-satunya beda adalah tidak ada padanan `ErrKeyCollision` (gambar QR tidak punya kunci/slug unik yang bisa bentrok). Webhook Kirimdev yang sama (`POST /public/webhooks/kirimdev`) di-fan-out ke `qrcoderequest_service.HandleWhatsAppReply` kalau balasan bukan milik permintaan shortlink. Detail endpoint: [API.md §8j](./API.md#8j-permintaan-qr-code-qrcode-requests).

---

## 13. Job Queue: Worker Persisten, Bukan Cron

`modules/jobqueue` adalah modul platform generik (pola sama seperti `modules/setting`: CMS-only, Super Admin only, tidak ada sisi publik) yang menggantikan goroutine fire-and-forget sebagai cara `shortlinkrequest_service` mengirim WhatsApp/email — retry dengan backoff, bukan sekali-coba.

**Worker adalah goroutine polling persisten di dalam proses yang sama, bukan proses terpisah yang di-cron.** Ini beda dari pola job-queue Laravel/PHP klasik (worker `php artisan queue:work` sebagai proses terpisah yang perlu di-manage cron/systemd/supervisor) karena `fsldk-api` sudah berupa satu proses Go yang long-running — tidak ada motivasi memisahkan worker jadi proses lain. `jobqueue_service.RunWorker(workerID)` dipanggil `JOBQUEUE_WORKER_COUNT` kali sebagai goroutine terpisah dari root `router.go` (`go jobqueueSvc.RunWorker(i)`), masing-masing polling `tr_job_queue` dengan interval `JOBQUEUE_POLL_INTERVAL_MS`.

**Dequeue aman-konkuren lewat claim atomik, pola yang sama dengan proteksi race approve/reject (§12).** `jobqueue_repository.Claim` melakukan `SELECT` kandidat job lalu `UPDATE ... WHERE jobID=? AND status='pending'` (dengan `attempts = attempts + 1` via `gorm.Expr`) — kalau `RowsAffected == 0`, worker lain sudah mengklaim job itu duluan, worker ini lanjut ke job berikutnya. Tidak perlu `SELECT ... FOR UPDATE SKIP LOCKED`.

**Retry dengan backoff, rate limiting per-queue, jitter anti-burst khusus WhatsApp.** Job yang gagal dieksekusi (`executeJob`) mendapat backoff `JOBQUEUE_BACKOFF_SCHEDULE_SECONDS` (default `15,30,60` detik) sampai `attempts >= maxAttempts`, baru jadi `status='failed'`. Queue `whatsapp` dibatasi `JOBQUEUE_WHATSAPP_RATE_PER_MINUTE` (default 8/menit) lewat `golang.org/x/time/rate.Limiter` — kalau limiter menolak, worker melewati queue itu tick ini (bukan kegagalan job, tidak menambah `attempts`). Setiap job WhatsApp juga dapat jitter acak 0-8 detik saat di-enqueue untuk menghindari burst pengiriman serentak.

**Stuck-job sweeper adalah pengganti graceful shutdown yang tidak ada.** Worker goroutine tidak punya penanganan `SIGTERM`/shutdown bersih — kalau proses `fsldk-api` crash saat sebuah job `status='processing'`, job itu akan macet selamanya kalau tidak ada yang membereskannya. `jobqueue_service.RunStuckSweeper()` (satu goroutine, `go jobqueueSvc.RunStuckSweeper()`) berjalan setiap 1 menit, me-recycle job yang macet di `processing` melewati `JOBQUEUE_STUCK_THRESHOLD_MINUTES` (default 10 menit) balik ke `pending` (kalau attempt masih tersisa) atau langsung `failed` (kalau sudah habis).

**`tr_whatsapp_message_log` dicatat oleh worker, dipakai fitur lain.** Setiap kali job `jobType=whatsapp_template` sukses terkirim, worker mencatat `waMessageID → correlationType/correlationID` ke tabel ini (`jobqueue_repository.LogOutboundMessage`) — dipakai `shortlinkrequest_service.resolveTargetRequest` (§12) untuk resolusi reply-threading balasan WhatsApp, tapi didesain generik (`correlationType` bebas) supaya fitur lain yang nanti kirim WhatsApp lewat job queue yang sama bisa memakai mekanisme resolusi serupa tanpa tabel baru.
